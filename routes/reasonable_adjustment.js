...
async function getReasonableAdjustmentsByActivityID(req, res, next) {
    try {
        ...
        const reasonableAdjustments = await pool.query("SELECT * FROM reasonable_adjustment WHERE activity_id = ?", [
            id,
        ])
        if (reasonableAdjustments[0].length === 0)
            return res.status(404).json({ message: "No reasonable adjustments found", status: 404 })
